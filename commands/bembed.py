...
class EmbedButton(discord.ui.View):
    ...
    @discord.ui.button(label="Send Embeds", style=discord.ButtonStyle.green)
    async def send_embeds(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("Sending embeds...", ephemeral=True)
...
async def bembedspam_command(interaction: discord.Interaction, count: int = 10):
    await interaction.response.send_message(
        f"Click the button to send {count} embeds!",
        view=EmbedButton(count),
        ephemeral=True
    )
